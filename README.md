...
Write a SQL query to calculate the total sales (total_sale)/(total_qty) for each category.:
select
category, 
sum(quantiy) as total_sale_qty,
sum(total_sale) as total_sale_value from retail_sales 
group by category;
Write a SQL query to find the average age of customers who purchased items from the 'Beauty' category and group by gender:
select gender,
round(avg(age)) as avg_age_of_customer 
from retail_sales 
...
Email: (imran.chowdhury099@gmail.com)
